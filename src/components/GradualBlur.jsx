...
const GradualBlur = props => {
  ...
  if (config.exponential) {
    blurValue = Math.pow(progress, 3) * currentStrength; 
  } else {
  ...
  const feather = 25; 
  const center = (i / config.divCount) * 100;
  
  const p1 = Math.max(0, center - feather);
  const p2 = center;
  const p3 = Math.min(100, center + feather);

  let gradient = `transparent ${p1}%, black ${p2}%, transparent ${p3}%`;
